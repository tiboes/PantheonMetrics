...
{
  private PanelM _experienceTogglePanel;
  private int _x;
  private int _y;
  private int _width;
  private int _height;
  ...
  public GuiExperience(int x, int y, int width, int height) 
  { 
    _x = x;
    _y = y;
    _width = width;
    _height = height;
    _experienceTogglePanel = new PanelM(0, _x, _y, _width, _height, GUIGlobals.GetButtonStyle(ButtonStyleEnum.Box), "Experience");
  }

  public void EnsureInitialialization()
  {
    if (_experienceTogglePanel == null)
      _experienceTogglePanel = new PanelM(0, _x, _y, _width, _height, GUIGlobals.GetButtonStyle(ButtonStyleEnum.Box), "Experience");
  }

  public void Render()
  {
    _experienceTogglePanel.ResetTextDefinitions();
    _experienceTogglePanel.SetGUIStyle(GUIGlobals.GetPanelStyle(PanelStyleEnum.Box));
    _experienceTogglePanel.AddText(new TextDefinition("EXPERIENCE", "Red", 20, true));
    _experienceTogglePanel.AddText(new TextDefinition("", "White", 20, true));//Just to make space
    _experienceTogglePanel.AddText(new TextDefinition("", "White", 20, true));//Just to make space
