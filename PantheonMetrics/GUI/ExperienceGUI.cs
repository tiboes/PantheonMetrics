...
{
  private static UnityEngine.Rect _debugWindowRect = new UnityEngine.Rect(12, 50, 300, 130);
  ...
  public static void Render()
  {
    ...
    if (UnityEngine.GUI.Button(rectCloseOpenBtn, "", buttonStyleActivated))
    {
    ...
    {
      if (UnityEngine.GUI.Button(rectBtn1, "<color=White>Reset</color>", buttonStyleSubButton))
      {
      ...
      UnityEngine.GUI.Box(rectHeader, GetGUIText($"Pantheon Metrics {PantheonMetricsMain.ModVersion}", "Red", (int)(lineH - 1), true), boxStyle);
      UnityEngine.GUI.Box(rectExpLastPeriod, GetGUIText($" Total 10 min: {MetricsExperience.TotalExperienceTheLast10MinCached.ToString("N0", CultureInfo.CurrentUICulture)}", "White", (int)(lineH - 5), true), boxStyle);
      UnityEngine.GUI.Box(rectExpAvt, GetGUIText($@" Average/min: {MetricsExperience.ExperiencePerMinTheLast10MinCached.ToString("N0", CultureInfo.CurrentUICulture)}", "White", (int)(lineH - 5), true), boxStyle);
      ...
      for (int i = 0; i < lastKillCount; i++)
      {
        ...
        UnityEngine.GUI.Label(rectExpEntry, GetGUIText($@"{entry.time: HH:mm:ss}: {entry.enemy}({entry.experience.ToString("N0", CultureInfo.CurrentUICulture)})", "White", (int)(lineH - 5)));
      }
  ...
  private static string GetBoldStartTag(bool isBold) => isBold ? "<b>" : "";
  private static string GetBoldEndTag(bool isBold) => isBold ? "<b>" : "";
